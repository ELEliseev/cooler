...
#Остается сделать так, чтобы скрипт,
#контролирующий работу вентилятора охлаждения,
#автоматически запускался каждый раз при загрузке системы.	
...
try:
    ...
    controlPin = 24                       # Пин отвечающий за управление
    ...
    p=GPIO.PWM(controlPin,100)
    while True:                             # Бесконечный цикл запроса температуры
        temp = get_temp()                   # Получаем значение температуры

        if temp > tempOn and not pinState:
            pinState = True         # Меняем статус состояния
            p.start(50) # Задаем новый статус пину управления
        if temp < tempOn - 10 and pinState:
            pinState = False        # Меняем статус состояния
            p.stop() # Задаем новый статус пину управления
        print(str(temp) + "  " + str(pinState)) # Выводим температуру в консоль
        sleep(10)                            # Пауза - 1 секунда
    ...
    print("CleanUp")
    p.stop()# Информируем о сбросе пинов
